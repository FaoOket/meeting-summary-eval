...
|---|---|
| Réunions évaluées | 7 |
| Couverture moyenne des points clés | Élevée |
| WER moyen (qualité de transcription) | 17 % |
| Latence médiane par réunion | 16 s |
| Fidélité (LLM-as-judge) | Limite connue — le juge est un LLM léger, à renforcer (voir *Limites & pistes*) |
...
Le pipeline (Whisper → Qwen) peut aussi se tester directement sur son propre
audio, sans passer par le harnais d'évaluation :
...
L'interface permet de déposer un fichier audio de réunion (`.m4a`, `.mp3`,
`.wav`, `.ogg`) et affiche la transcription brute, le résumé structuré
(Points clés / Décisions / Actions) ainsi que les temps de traitement
(transcription + résumé). Un exemple d'audio réel est fourni dans
`audio/exemple_reunion.m4a` pour tester sans avoir à enregistrer soi-même.
...
├── audio/              # enregistrements (exemple_reunion.m4a versionné, le reste non)
